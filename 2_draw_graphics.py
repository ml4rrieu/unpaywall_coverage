...
# perYear = len( df.loc[df['Year'] == 2016.0])
# print(perYear)
perYear = [len(
	df.loc[ df['Year'] == float(y)]) for y in range(2014, 2018)
]
...
print(f"sum all no DOIs {sum(noDois)}")
print(f"sum dois not in upw {sum(doisNotInUpw)}")
